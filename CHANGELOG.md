...
## 1.0.10
- Changed: the data file is written on a thread of its own, so the game no longer waits for the disk. On Windows that write measured 8ms of a 9.6ms step on a fast machine, most likely antivirus looking at each new file, against about 1.3ms for everything else the mod does. If writes fall behind, only the newest data is written.
- Changed: the zone's name is asked of the game on entering a zone and then every few seconds, instead of on every update. The game builds it from its world and naming data, which measured half a second the first time in a session. A place you discover while standing in it shows its new name within five seconds. Player.log also gets one note per session with what an ordinary step costs the mod, averaged over the first fifty after loading.
...
- Fixed: travelling on the world map was noticeably slower with the mod enabled, up to a second between a keypress and the move. One step there passes hundreds of game turns, and the mod rebuilt and rewrote its data on every one of them. It now updates once when you get control back, and a few times a second while time passes without input, as when resting. A step that still costs noticeable time is noted in Player.log with the parts it went on, and so is the first update of a session, since that one also finds everything the mod reads from the game.
