...
- Fixed: travelling on the world map was noticeably slower with the mod enabled, up to a second between a keypress and the move. One step there passes hundreds of game turns, and the mod rebuilt and rewrote its data on every one of them. It now updates once when you get control back, and a few times a second while time passes without input, as when resting. A step that still costs noticeable time is noted in Player.log, and so is the first update of a session, part by part, since that one also finds everything the mod reads from the game.
